    ...
    <div className="max-w-6xl mx-auto mb-24 px-4">
      ...
      >
        <div className="grid grid-cols-4">
          {/* Sidebar */}
          <div className={cn("col-span-1 border-r p-4", "border-white/10")}>
            <div className="flex items-center justify-between p-2 mb-4">
              <h3 className="text-white/70 font-semibold">Hanzo Team</h3>
              <ChevronDown className={cn("w-4 h-4", "text-white/50")} />
            </div>
            <div className="space-y-2">
              {channels.map((channel) => (
                ...
                >
                  <Hash className={cn("w-4 h-4", "text-white/50")} />
                  <span className={cn("text-sm", "text-white/70")}>{channel}</span>
                </motion.div>
              ))}
            </div>
          </div>
          ...
          <div className="col-span-3 p-4">
            <div className={cn("flex items-center gap-4 p-4 border-b mb-4", "border-white/10")}>
              <Hash className={cn("w-5 h-5", "text-white/50")} />
              <span className={cn("font-medium", "text-white")}>general</span>
            </div>
            <div className="space-y-4">
              {messages.map((msg, idx) => (
                ...
                >
                  <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-white/30 to-white/10 flex items-center justify-center text-white text-sm">
                    {msg.user[0]}
                  </div>
                  <div>
                    <div className="flex items-center gap-2">
                      <span className={cn("font-medium", "text-white")}>{msg.user}</span>
                      <span className={cn("text-xs", "text-white/50")}>{msg.time}</span>
                    </div>
                    <p className={cn("text-white/70")}>{msg.message}</p>
                  </div>
                </motion.div>
              ))}
            </div>
