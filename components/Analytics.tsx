...
/** Publishable ingest key (pk_…), minted per org via POST /v1/ingest/keys. This is
 *  a marketing site: every visitor is logged out, so no bearer can ride the
 ...
 * route change. Children render inside so any page can reach the same client via
 * useAnalytics() — errors are just events on that one stream, so there is no
 * separate DSN.
 */
export function Analytics({ children }: { children: React.ReactNode }) {
  return (
    <AnalyticsProvider
      config={{ product: 'hanzo-industries', host: HOST, ingestKey: INGEST_KEY, enabled: consented() }}
    >
      <Pageview />
      {children}
    </AnalyticsProvider>
